...
# --- 1. 페이지 스타일 및 정렬 ---
st.set_page_config(page_title="보안팀 릴리즈 아카이브 Pro v28.0", layout="wide")
...
cursor = conn.cursor()
# 3.1.3 버전에 맞춰 컬럼 구조는 유지하되 통합 저장을 위해 improvements 필드를 주로 사용합니다.
...
# --- 3. [핵심] 통합 추출 파싱 로직 (3.1.3 & 3.1.4 공용) ---

def parse_pdf_integrated(file):
    with pdfplumber.open(file) as pdf:
        ...
        for page in pdf.pages:
            ...
            # 표 데이터 추출 및 행 단위 결합 (짤림 방지)
            tables = page.extract_tables()
            for table in tables:
                for row in table:
                    # 셀 내부 줄바꿈 제거 후 한 줄로 병합
                    cells = [str(c).replace('\n', ' ').strip() for c in row if c]
                    if len(cells) >= 2:
                        # 3.1.3 표: [구분, 모듈, 상세내용, 이슈번호]
                        # 3.1.4 표: [구분, 카테고리, 내용...]
                        # 구분 컬럼이 '개선', '이슈', '신규', 'BUG', 'TASK' 등일 때 수집
                        if any(kw in cells[0] for kw in ['개선', '이슈', '신규', 'BUG', 'TASK', '수정']):
                            mod = cells[1]
                            desc = cells[2]
            ...
            # 일반 텍스트에서도 [항목] 또는 • 기호 탐색
            # 표로 잡히지 않는 텍스트 항목들을 보충
            clean_p = re.sub(r'\s+', ' ', p_text)
            text_items = re.findall(r'(\[[^\]]+\][^\[•]+|•[^\[•]+)', clean_p)
            for ti in text_items:
                if len(ti.strip()) > 5: # 너무 짧은 기호 제외
                    all_items.append(ti.strip())
    ...
    return {
        ...
        "all_content": "\n".join([f"* {item}" for item in dict.fromkeys(all_items)]), # 중복 제거 및 * 추가
        "raw_text": full_raw
    }

# --- 4. 사이드바 (모든 메뉴 복구) ---
if 's_key' not in st.session_state: st.session_state.s_key = "v28"
...
# --- 5. 메인 화면 ---
st.title("🛡️ TrusGuard 통합 관제 (v28.0)")
...
if keyword:
    ...
    st.markdown(f"""<div class='report-card'>
        ...
        <span class='sub-label'>📋 상세 변경 내역 (통합)</span>{r['improvements'].replace('\n','<br>')}
